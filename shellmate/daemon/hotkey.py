...
def on_activate():
    """
    Spawn the TUI popup in a separate subprocess.
    This avoids the 'ValueError: signal only works in main thread' crash
    caused by running the Textual app inside a pynput daemon thread.
    """
    try:
        # Run using the same python interpreter executing this daemon
        subprocess.run([
            sys.executable,
            "-c",
            "from shellmate.tui.popup import run_popup; run_popup()"
        ])
    except Exception as e:
        print(f"[shellmate-daemon] Failed to launch TUI: {e}")
